function  [Averaged_Models, Adjacency_Matrix, Edges_Inhibition_Activation]= Occams_Model_Proposal_with_F_and_Steel_Sort_Berno_Predictors_Steady(mRNA_to, dmRNA_dt, Candidates, Prior)
...
for j = 1: size(dmRNA_dt,2)
    ...
    j
               ...
               %% Step 1: Calculate the "Null" model's Bayes Factor
                ...
                Design_Matrix = [ ones(size(mRNA_to,1),1)];
                ...
                Active = 1;
                      ...
                      %% Step 2: Add the Next Gene in the List
                                      ...
                                      I=find(Candidates_Pred(1,:)==Root(l,end));
                                          if Root(l,:) == 0;
                                          Branches = Candidates_Pred(1,:);
                                          else
                                          Branches = Candidates_Pred(1,I+1:end);
                                          end
                                                  ...
                                                  for k = 1:length(Branches)
                                                      ...
                                                      D_M = [Predictors(:, Branches(k)) ones(size(mRNA_to,1),1)];
                                                      p = size(D_M,2)-1;
                                                      
                                                      else 
                                                          
                                                          D_M = [Predictors(:,[Root(l,:) Branches(k)])  ones(size(mRNA_to,1),1)];
                                                      p = size(D_M,2)-1;
                                              ...
                                              %%  Define the State of the search (if none models passed stop the search)
                                                             if  sum(New_Roots) == 0
                                                               ...
                                                               if Root == 0
                                                                   ...
                                                                   Sorted_TFs(counter+1:end) = setdiff(Candidates_Pred(1,:), New_Roots(1:counter));
                                                                   
                                                                   Candidates_Pred(1,:) = Sorted_TFs;
                                                               end
